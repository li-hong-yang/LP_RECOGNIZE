...
def parse_arg():
    ...
    parser.add_argument('--checkpoint', type=str,
                        ...
                        # default='/home/hyli/project/OCR/CRNN_Chinese_Characters_Rec/output/OWN/crnn/2020-12-08-21-59/checkpoints/checkpoint_199_acc_0.8085.pth',
...
def recognition(config, img, model, converter, device,image_name=None):
    ...
    model.eval()
    preds = F.softmax(model(img)[2],dim=-1)
    # print(preds.shape)
    _, preds = preds.max(2)
    preds = preds.transpose(1, 0).contiguous().view(-1)

    sim_pred = converter.SRN_test_decode(preds)
